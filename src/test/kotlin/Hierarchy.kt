...
fun Hierarchy.filter(nodeIdPredicate: (Int) -> Boolean): Hierarchy {
    ...
    var filteringInitialDepth = -1;

    for (index in 0 until size) {
        val nodeId = nodeId(index)
        val depth = depth(index)

        if (filteringInitialDepth == -1 || depth <= filteringInitialDepth) {
            if (nodeIdPredicate(nodeId)) {
                filteredNodeIds.add(nodeId)
                filteredDepths.add(depth)
                filteringInitialDepth = -1
            } else {
                filteringInitialDepth = depth
            }
        }
    }
